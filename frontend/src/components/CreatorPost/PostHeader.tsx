...
export default function PostHeader({ title, creatorId }: PostHeaderProps) {
  return (
    <div className="flex flex-col gap-[20px] items-start w-full max-w-[1119px] shrink-0">
      
      {/* Back Button */}
      <Link href={creatorId ? `/user/creators/${creatorId}` : "/user/creator"} className="bg-[#faf8f5] border border-[#d8d1c7] flex gap-[4px] items-center justify-center px-[8px] py-[4px] rounded-[36px] transition-colors hover:bg-[#f6f4f1]">
      ...
      <h1 className="font-['Fjalla_One',sans-serif] font-normal leading-[57.6px] text-[#1a1a1a] text-[40px] tracking-[0.8px] w-full max-w-[1116px]">
        {title || "Transform your body in just 30 days with simple home workouts and a guided nutrition plan."}
      </h1>
